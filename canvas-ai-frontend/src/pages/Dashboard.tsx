...
const PAGE_META: Record<string, { icon: typeof MessageCircle; tint: string; label: string }> = {
  learn: { icon: MessageCircle, tint: 'text-ink-soft bg-paper-deep border-line', label: 'Learn' },
  chat: { icon: MessageCircle, tint: 'text-ink-soft bg-paper-deep border-line', label: 'Chat' },
  practice: { icon: Target, tint: 'text-ink-soft bg-paper-deep border-line', label: 'Practice' },
  quiz: { icon: Target, tint: 'text-ink-soft bg-paper-deep border-line', label: 'Quiz' },
  exam: { icon: ClipboardList, tint: 'text-ink-soft bg-paper-deep border-line', label: 'Exam' },
  exams: { icon: ClipboardList, tint: 'text-ink-soft bg-paper-deep border-line', label: 'Exam' },
  kit: { icon: Layers, tint: 'text-ink-soft bg-paper-deep border-line', label: 'Study Kit' },
  notes: { icon: Layers, tint: 'text-ink-soft bg-paper-deep border-line', label: 'Notes' },
  progress: { icon: BarChart3, tint: 'text-ink-soft bg-paper-deep border-line', label: 'Progress' },
}
const pageMeta = (page: string) =>
  PAGE_META[page] ?? { icon: BookOpen, tint: 'text-ink-soft bg-paper-deep border-line', label: page }
...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-9 space-y-9">
      ...
      >
        <div>
          <h1 className="text-[2rem] leading-tight font-semibold tracking-tight text-ink font-display">{getGreeting()}</h1>
          <p className="text-sm text-ink-soft mt-1.5">{today}</p>
        </div>
        {hasCourses && (
            ...
            {recentWithNames.map((entry) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-medium text-ink truncate">{entry.courseTitle}</div>
                    <div className="text-xs text-ink-faint mt-0.5">{m.label} · {timeAgo(entry.timestamp)}</div>
                  </div>
                  <ArrowRight className="w-4 h-4 text-ink-faint group-hover:text-accent group-hover:translate-x-0.5 flex-shrink-0 transition-all" />
                </Card>
              )
